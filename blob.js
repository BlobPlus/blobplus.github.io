...
var BlobWorld = new function() {
	...
	this.init = function() {
		...
		if (canvas && canvas.getContext) {
			...
			createBlob( { x: worldRect.width*0.15, y: worldRect.height*(Math.random()*0.2) }, { x: 0, y: 0 } );
			createBlob( { x: worldRect.width*0.85, y: worldRect.height*(Math.random()*0.2) }, { x: 0, y: 0 } );
